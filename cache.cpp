...
{
    fstream out("memory.txt", ios::out | ios::trunc);
    for (int i = 1; i <= 1e5; i++)
    {
        out << i << " " << data[i];
        if (i != 1e5)
...
public:
    ...
    CacheBlock selectBlock(CacheBlock, int, int);
...
int Cache::readFromCache(int address)
{
    ...
    CacheBlock newBlock = selectBlock(currBlocks[pos], reqTag, reqIndex);
...
CacheBlock Cache::selectBlock(CacheBlock oldBlock, int reqTag, int reqIndex)
{
    // cout << "READFROMMEMORY" << endl;
...
CacheBlock Cache::selectBlock(CacheBlock oldBlock, int reqTag, int reqIndex, int address, int data)
{
    ...
    if (oldBlock.tag != -1 && oldBlock.st == M)
    {
        int tagPlusIndexOld = (oldBlock.tag) * numberOfSets + reqIndex;
        for (int i = 0; i < numberPerBlock; i++)
            dataMem.writeMem(tagPlusIndex * numberPerBlock + i, oldBlock.data[i]);
    }
    return newBlock;
}
void Cache::closeMem()
